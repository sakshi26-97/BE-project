...
#Region "Friend Function WriteAggregatePrecipitationData(ByVal filePath As String, ByVal aggType As AggregateType) As String"
    Friend Function WriteAggregatePrecipitationData(ByVal filePath As String, ByVal aggType As AggregateType) As String
        ...
        Dim strFullPath As String = filePath
        strFullPath += _strFileName & "_SPI_Aggregate_"
        strFullPath += aggType.ToString().Substring(0, 1)
        ...
        Using sw As StreamWriter = New StreamWriter(strFullPath)
            ...
            sw.WriteLine("""" & _strFileHeader & """")
            sw.WriteLine("""date"", ""precip""")

            If Precipitation IsNot Nothing AndAlso Precipitation.Any() Then

                For Each Val As KeyValuePair(Of Date, Double) In Precipitation

                    sw.WriteLine("""" & Val.Key.ToString(_strDateFormat) & """, """ & Val.Value.ToString(_nfiDecimalFormat) & """")

                Next

            Else

                sw.WriteLine("No Precip found.")
...
#Region "Private Sub CompoundToWeeklyData()"
    Private Sub CompoundToWeeklyData()
        ...
        For i As Integer = 0 To intYears.Count - 1
            ...
            Dim lnqPrecipDataByYear = (From n In dicPrecip Where n.Key.Year = intYears(iterationvar) Order By n.Key).ToList()

            '*** data is good, aggregate by every 7 days
            Dim dblPrecip As Double = StandardPrecipitationIndex.Missing(0)
            ...
            While j < lnqPrecipDataByYear.Count() - 1
                ...
                For k As Integer = 0 To intDays - 1
                    ...
                    If Not StandardPrecipitationIndex.Missing.Contains(lnqPrecipDataByYear(j + k).Value) Then

                        If StandardPrecipitationIndex.Missing.Contains(dblPrecip) Then
                ...
                intDays = 0
                dblPrecip = StandardPrecipitationIndex.Missing(0)
...
#Region "Private Sub CompoundToMonthlyData()"
    Private Sub CompoundToMonthlyData()
        ...
        '*** getting a list of all years
        Dim intYears As List(Of Integer) = dicPrecip.Select(Function(x) x.Key.Year).Distinct().ToList()

        For Each DataYear In intYears

            '*** getting all data for the year
            Dim lnqPrecipDataByYear = From n In dicPrecip Where n.Key.Year = DataYear Order By n.Key

            '*** data is good, aggregate by month
            For j As Integer = 1 To 12

                Dim dblPrecip As Double = StandardPrecipitationIndex.Missing(0)
                Dim DataMonth As Integer = j

                '*** getting all days in a month
                Dim lnqPrecipDataByMonth = From n In lnqPrecipDataByYear Where n.Key.Month = DataMonth Order By n.Key

                For Each day In lnqPrecipDataByMonth

                    If Not StandardPrecipitationIndex.Missing.Contains(day.Value) Then
                ...
                Precipitation.Add(Convert.ToDateTime(DataMonth & "/1/" & DataYear, SPICollection.Culture), dblPrecip)
